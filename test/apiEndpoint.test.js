  ...
  describe("stations", () => {
    it("should return a response with the expected JSON structure.", (done) => {
      ...
      const stationEndpoint =
        "https://www.mvg.de/api/fahrinfo/location/query?q=Hauptbahnhof";
      ...
      request.get(
        ...
        (error, response, body) => {
          ...
          assert.isArray(
            bodyJson.locations,
            "Locations array is not present ."
          );
          assert.isAbove(
            bodyJson.locations.length,
            0,
            "No location found in the returned JSON."
          );
          ...
          assert.containsAllKeys(
            station,
            ["id"],
            "JSON for station did not contain station id."
          );
    ...
    it("should return a response with the expected JSON structure.", (done) => {
      ...
      needle.get(
        ...
        (error, response) => {
          ...
          assert.isArray(
            bodyJson.departures,
            "Departures array is not present ."
          );
          assert.isAbove(
            bodyJson.departures.length,
            0,
            "No departures found in the returned JSON."
          );
          ...
          assert.containsAllKeys(
            departure,
            expectedKeys,
            "JSON for departure did not contain station id."
          );

          const product = departure.product;
          assert.oneOf(
            product,
            ["UBAHN", "SBAHN", "BUS", "REGIONAL_BUS", "TRAM"],
            "The type of transportation was of unknown kind."
          );
